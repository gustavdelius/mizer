...
runnit <- function(par=mypar){
  ...
  source("./R/Experimental Code/project_methodsmodPREYSWITCH2.R")
  source("./R/Experimental Code/projectmodPREYSWITCH2.R")
  sim <- project(params, effort = 1, dt = 0.1, t_save =1, initial_n=init_n,initial_n_pp=init_n_pp,t_max=25)
...
op <- optim(par=opout, fn=minmesimpler, method = "SANN", control = list(maxit = 1000))
opout <- op$par
opout
mypar
simm <- runnit(opout)
...
landout <- function(par=mypar){
  simm <- runnit(par)
  gy <- getYield(simm)
  print(log(landings_data90+10^(-10)))
  print(log(gy[dim(gy)[1],]*10^(-6)+10^(-10)))
}
# stella is smart and got this
landout(c(24.281402, c(-22.56984, -24.47867, -21.29554, -23.94145, -22.96104, -22.05618, -23.20668,
  -22.29771, -21.71369, -22.20643, -21.99999, -21.99000)))

minmesimpler(c(24.28143, c(-22.56984, -24.47867, -21.29554, -23.94145, -22.96104, -22.05618, -23.20668,
                           -22.29771, -21.71369, -22.20643, -21.99999, -21.72246)))

minmesimpler(opout)
minmesimpler(c(24.281402, c(-22.56984, -24.47867, -21.29554, -23.94145, -22.96104, -22.05618, -23.20668,
                            +   -22.29771, -21.71369, -22.20643, -21.99999, -21.99000)))
landout(c(24.281402, c(-22.56984, -24.47867, -21.29554, -23.94145, -22.96104, -22.05618, -23.20668,
                       -22.29771, -21.71369, -22.20643, -21.99999, -21.99000)))
