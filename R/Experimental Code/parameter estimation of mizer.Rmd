...
We shall make a `gdplot()` based procedure to allow us to visualize such time series:
...
We can see that there is quite a lot of measurement noise by comparing with the corresponding error free time series:

```{r}
show_time_series(model(mypar))
```
...
We shall investigate the minima of this cost function over the entire parameter space, before restricting our attention to a subspace. The actual parameter value used to generate the data was `mypar`.

```{r}
mypar
```

To investigate whether this is a minima I ran the `optim` function, starting at this point.

```{r}
myOP<-optim(fn = fish_model_cost, par=mypar,YY=Y, sd=mysd, method="BFGS")
myOP2<-optim(fn = fish_model_cost, par=myOP$par,YY=Y, sd=mysd, method="BFGS")
```

I believe this run stops at the nearby local minimum. Our results at least verify it is a fixed point of `optim`.
```{r}
myOP$par
myOP2$par
```

Let us start our optimization algorithm from a different point. 

```{r}
diffOP <- optim(fn = fish_model_cost, par=rep(9, length(mypar)),YY=Y, sd=mysd, method="BFGS")
diffOP2 <- optim(fn = fish_model_cost, par=diffOP$par,YY=Y, sd=mysd, method="BFGS")
```


In this case optim converges to a different fixed point, suggesting that our likelihood function is multimodal:
...
Let us compare the model costs at `mypar`, the optima `myOP$par` near `mypar`, the alternative starting point `rep(9, length(mypar))` and the fixed point `diffOP$par` which `optim` moves this alternative starting point to:

```{r}
ans <- c(fish_model_cost(par=mypar),
  fish_model_cost(par=myOP$par),
  fish_model_cost(par=rep(9, length(mypar))),
  fish_model_cost(par=diffOP$par))
```
...
hist(MCMC)
```
Now we will use the old model run data, together with the a new time series made up by a different model. Essentially we changed the settings we used to generate the data, so that the true generator lies outside the considered model space. Despite this we shall attempt to generate a likelihood surface:
...
mysd2 <- 1
mypar2 <- mypar
mypar2[3:length(mypar2)] <- rep(min(mypar),(length(mypar2)-2))
mypar2
XX2 <- model(mypar2)
Y2 <- XX2+rnorm(prod(dim(XX2)),mean = 0,sd=mysd2)

time_series_to_cost <- function(timedata=Y,YY=Y, sd=mysd){
  return(sum((timedata-YY)^2/(sd^2)))
}
RDC2 <- array(0, c(length(log_carrying_capacity),length(log_r_max)))
for (i in (1:length(log_carrying_capacity))){
  for (j in (1:length(log_r_max))){
    RDC2[i,j] <- time_series_to_cost(timedata=RD[i,j, ,],YY=Y2, sd=mysd2)
  }
}

```

We can plot this likelihood surface

```{r}
persp3D(x=log_carrying_capacity, y=log_r_max, z=exp(-RDC2/2),xlab = "log_carrying_capacity",
        ylab = "log_r_max", zlab = "likelihood")
```

The high region of the likelihood function shown in this contour plot does not contain the actual values used, although the used values are not that far from the maximum of the likelihood function. The discrepancy is because different, inaccessible, settings were used to generate the data `Y2`, in this case.
...
```{r}
contour(x=log_carrying_capacity, y=log_r_max, z=exp(-RDC2/2),xlab = "log_carrying_capacity",
        ylab = "log_r_max")

```

Let us repeat this kind of experiment using different external model settings:
...
mysd2 <- 1
mypar2 <- mypar
#mypar2[3:length(mypar2)] <- runif((length(mypar2)-2),8,9)
mypar2[3:length(mypar2)] <- rep(10,(length(mypar2)-2))
mypar2
XX2 <- model(mypar2)
Y2 <- XX2+rnorm(prod(dim(XX2)),mean = 0,sd=mysd2)
...
```
Let us plot the likelihood surface in this case
