...
setMethod('project', signature(object='MizerParams', effort='array'),
    function(object, effort, t_save=1, dt=0.1, initial_n=get_initial_n(object), initial_n_pp=object@cc_pp,  ...){
        ...
        for (i_time in 1:t_steps){
            ...
            #B[,idx] <- 1 + sweep(e_growth[,idx,drop=FALSE]*dt,2,sim@params@dw[idx],"/") + z[,idx,drop=FALSE]*(n[,idx,drop=FALSE]^epsi)*dt
            B[,idx] <- 1 + sweep(e_growth[,idx,drop=FALSE]*dt,2,sim@params@dw[idx],"/") + z[,idx,drop=FALSE]*(sweep(n[,idx,drop=FALSE],2,(kappaR2*sim@params@w[idx]^(-lambda2)),"/")^(epsi))*dt
            # S_{ij} <- N_i(w_j)
            
            
            S[,idx] <- n[,idx,drop=FALSE]
            # Boundary condition upstream end (recruitment)
            #B[w_min_idx_array_ref] <- 1+e_growth[w_min_idx_array_ref]*dt/sim@params@dw[sim@params@species_params$w_min_idx]+z[w_min_idx_array_ref]*(n[w_min_idx_array_ref]^epsi)*dt
            B[w_min_idx_array_ref] <- 1+e_growth[w_min_idx_array_ref]*dt/sim@params@dw[sim@params@species_params$w_min_idx]+z[w_min_idx_array_ref]*((n[,1]/(kappaR2*sim@params@w[1]^(-lambda2)))^(epsi))*dt
