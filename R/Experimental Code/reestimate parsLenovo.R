...
model <- function(paras)
{
  ...
  dd$r_max <- rmax[1:(length(dd$r_max))]
  params <- MizerParams(dd, interaction = inter, kappa=capacity)
  sim_ini <- project(params, effort = t(Fmatlong), dt = 0.1, t_save =1, initial_n=myn)
  sim <- project(params, effort = t(Fmat3), dt = 0.1, t_save =1, initial_n=sim_ini@n[dim(sim_ini@n)[1],,],initial_n_pp=sim_ini@n_pp[dim(sim_ini@n_pp)[1],])
...
log_landings_short <- log_landings[24:44]

fish_model_cost <- function(par=mypar,YY=log_landings_short, sd=mysd){
...
MCMCrun111 <- MCMC
save(MCMCrun111, file="MCMCrun111.RData")
