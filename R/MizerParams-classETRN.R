...
setMethod('MizerParams', signature(object='numeric', interaction='missing'),
          function(object, min_w = 0.001, max_w = 1000, no_w = 100,  min_w_pp = 1e-10, no_w_pp = NA, species_names=1:object, gear_names=species_names){
            ...
            res <- new("MizerParams",
                       ...
                       selectivity=selectivity, mu_et=mu_et, catchability=catchability,
...
setMethod('MizerParams', signature(object='data.frame', interaction='matrix'),
          function(object, interaction,  n = 2/3, p = 0.7, q = 0.8, r_pp = 10, 
            ...
            res@mu_et <- (-betaval^(1 - lambdaval + qval))*exp((1/2)*((1 - lambdaval + qval)^2)*sigmaval^2)*(-1 + fbar)*gammaval*kappaval*sqrt(pi/2)*sigmaval*wvec^(1 - lambdaval + qval)*(sqrt(1/sigmaval^2)*sigmaval + erf(((1 - lambdaval + qval)*sigmaval^2 + log(betaval) + log(wvec) -log(WW))/(sqrt(2)*sigmaval)))
            #####
            #res@mu_et[1:7] <- c(lambda,WW,f0,kappa,qval,n,betaval)	
            return(res)
          }
)
