...
max(real_parts)
#library("RSpectra")
#EE2 <- eigs(jacobian_numeric, k=3, which = "LR")
#EE2$values

root_nmod <- root_n
root_nmod[1,] <- 1.1*root_nmod[1,]

simmod <- project(used_params, t_max=500, effort = 0, initial_n = root_nmod, t_save = 1, initial_n_pp = root_n_pp)
plot(simmod)


root_nmod2 <- root_n
root_nmod2[1,] <- 10*root_nmod2[1,]

simmod2 <- project(used_params, t_max=100, effort = 0, initial_n = root_nmod2, t_save = 1, initial_n_pp = root_n_pp)
plot(simmod2)

ss_go2 <- multiroot(f = change_function, start = c(simmod2@n_pp[dim(simmod2@n_pp)[1],],simmod2@n[dim(simmod2@n)[1],,]))

root_n_pp3 <- ss_go2$root[1:length(used_params@cc_pp)]
root_n3 <- array(ss_go2$root[(length(used_params@cc_pp)+1):length(ss_go2$root)], dim = dim(used_params@initial_n))

sim3 <- project(used_params, t_max=15, effort = 0, initial_n = root_n3, t_save = 1, initial_n_pp = root_n_pp3)
plot(sim3)

leading_vec <- sapply(EE$vectors[which.max(real_parts),], function(x) Re(x))

vec_n_pp <- leading_vec[1:length(used_params@cc_pp)]
vec_n <- array(leading_vec[(length(used_params@cc_pp)+1):length(leading_vec)], dim = dim(used_params@initial_n))

plot(used_params@w,vec_n[1,],log="x", type= "l")
