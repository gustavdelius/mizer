...
# sp$w_inf <- sp$adult_max_weight..calculated.
# sp$w_mat[1:11] <- sp$w_inf[1:11] / 4
sp$alpha <- 0.4
sp$beta[sp$beta == 1] <- 100
...
f0 <- 0.6
age_mat <- -log(1 - (sp$w_mat/sp$w_inf)^(1/sp$b)) / sp$k_vb
sp$h <- (sp$w_mat^(1 - n) - sp$w_min^(1 - n)) / age_mat / (1 - n) / 
    sp$alpha / (f0 - 0.2)
sp$ks <- 0.2 * sp$h * sp$alpha
# default for rho
...
ae <- sqrt(2 * pi) * sp$sigma * sp$beta^lm2 *
    ...
    (pnorm(3 - lm2 * sp$sigma) + 
         pnorm(log(sp$beta)/sp$sigma + 
                   lm2 * sp$sigma) - 1)
sp$gamma <- sp$h / (kappa * ae) * f0 / (1 - f0) * (1 - sp$detQ / 2 - sp$scavQ / 2)
...
params <- MizerParams(sp,
                      ...
                      q = q, n = n, lambda = lambda, kappa = kappa,
                      f0 = f0, z0pre = 0)
params@initial_B <- B
