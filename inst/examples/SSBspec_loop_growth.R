...
fg <- (p@species_params$beta!=100)

g <- getEGrowth(p,p@initial_n,p@initial_n_pp)[fg,]
if (kk==1) {
    g0 <- g
}

deviation_from_power_law[kk] <- sum(colSums((g-g0)^2)*p@dw) 
...
# I have put a calculator of the growth rate, as different of the foreground multiplier are used. 
# The y-axis gives sum_i sum_k (g_i^0 (w_k) - g_i (w_k))^2 dw_k which measure the deviation in growth 
# rates. Similar experiments could be done using the total death rate, by replacing 
# g <- getEGrowth(p,p@initial_n,p@initial_n_pp)[fg,]
########## with
# getMort(p,p@initial_n,p@initial_n_pp,effort = effort)[fg,]
