...
image(MMa, axes = FALSE,
      xlab = expression(cos(r^2)))


############ raster plot with ggplot

library(reshape2)
library(ggplot2)

m <- matrix(sample(1:6,100,replace=TRUE),nrow=10)
df <- melt(m)
df$value <- as.factor(df$value)
ggplot(df) + geom_raster(aes(fill=value,x=2*Var1,y=Var2))+
    scale_fill_manual(values = c("red","green","blue","yellow","pink",rgb(.115,.74,.18)))
...
#sapply((1:max(as.numeric(df$value))),function(x) rgb(x/max(as.numeric(df$value)),.74,.18))
#rgb(115,74,18)
