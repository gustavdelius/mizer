...
set_for_fishing <- function(knife_edge_size=10^2,effort=1,inter=inter){
  f0 <- 0.6
  alpha <- 0.4
  r_pp <- 1e-1
  n <- 2/3
...
effort <- 1
knife_edge_size <- 10^2
st <- set_for_fishing(knife_edge_size=knife_edge_size,effort=effort,inter=inter)
t_max <- 30
t_save <- t_max/100
...
effort <- 1
knife_edge_size <- 10^2
st <- set_for_fishing(knife_edge_size=knife_edge_size,effort=effort,inter=internull)
...
st$params@cc_pp <- (1+m2_background/params@rr_pp) * initial_n_pp
...
plot(times,GY[,no_sp]/GYIN[,no_sp],col=gray(1-(i/no_sp)),ylim=c(0.7,1.2),type="l")
for (i in ((no_sp-1):7)){
