...
\begin{cases}
\mathcal{N}w^{\alpha-\lambda} & w_{min} \leq w \leq w_{max}\\
0&\text{otherwise}
\end{cases}$$
The normalisation factor $\mathcal{N}$ is easy to calculate:
$$\mathcal{N}=\frac{w_{\text{max}}^{1+\alpha-\lambda}-w_{\text{min}}^{1+\alpha-\lambda}}
{1+\alpha-\lambda}.$$
The density function for the log of the prey size is therefore
$$f_{\log(W)}(x)=
\begin{cases}
\mathcal{N}\exp\left((1+\alpha-\lambda)x\right) & \log(w_{min}) \leq x \leq \log(w_{max})\\
0&\text{otherwise}
\end{cases}$$