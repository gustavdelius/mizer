...
# from running rescaled

Xbest <- n2RECvec(params, n_pp=root_n_pp, n=root_n)



FF <- function(XX){
  no_sp <- dim(params@initial_n)[1]
  YY <- c(Xbest[1:(length(params@cc_pp)+no_sp)],XX)
  theout <- myF_rescaled(params,YY)
  return(theout[(length(params@cc_pp)+no_sp+1):length(theout)])
}
...
wtss <- params@w^(params@lambda)

trial_data_in <- wtss*colSums(params@initial_n)

ss <- multiroot(f = FF, start = trial_data_in, rtol = 0, atol = 1e-15, ctol = 0)
no_sp <- dim(params@initial_n)[1]
outvec <- c(Xbest[1:(length(params@cc_pp)+no_sp)],ss$root)/get_wts(params)
outputs <- RECvec2n(params, outvec)
...
sim <- project(params, t_max=15, effort = 0, initial_n = RECvec2sols(params, outvec), t_save = 1, initial_n_pp = outputs$n_pp)
plot(sim)
