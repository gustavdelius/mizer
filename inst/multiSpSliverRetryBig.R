...
true_sol <- sol
for (i in 1:no_s){
    sol_mult[i] <- sol_mult[1]*(params@species_params$w_mat[1]/params@species_params$w_mat[i])^(params@lambda+1)
...
plot(params@w,true_sol[2,],log="xy",type="l")
lines(params@w,sim@n[dim(sim@n)[1],2,],col="blue")


plot(params@w,true_sol[1,],log="xy",type="l",ylim=c(10^(-20),10^(-5)))
