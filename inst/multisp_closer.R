# got Gustav's code from Dec 13, sorted out theta activation, and 
# set Rmax using params2. I also use 20 species instead of 10. These settings 
# look closer to goal, but improvement is wanted

# An example where our analytic steady state evolves to a different steady state
# solution with a different ordering in the abundance of species.
...
myrpp <- 10^(18)
myno_sp <- 20
myinteraction <- matrix(0,nrow=myno_sp,ncol=myno_sp)
myno_w <- 1000
#params <- set_trait_model(no_sp = 10, min_w_inf = 10, max_w_inf = 10^5,
#                          w_pp_cutoff = 10^5,z0pre = 0,p=2/3,alpha=myalpha,no_w=1000,q=myq)
params <- set_trait_model(interaction=myinteraction,r_pp = myrpp,no_sp = myno_sp, min_w_inf = 10, max_w_inf = 10^5,
                          w_pp_cutoff = 10^5,z0pre = 0,p=2/3,alpha=myalpha,no_w=myno_w,q=myq)
#params <- set_trait_model(interaction=myinteraction,r_pp = myrpp,no_sp = myno_sp, min_w_inf = 10, max_w_inf = 10^5,
#                          w_pp_cutoff = 10*0.25,z0pre = 0,p=2/3,alpha=myalpha,no_w=1000,q=myq)
...
true_sol <- sol
for (i in 1:no_s){
    sol_mult[i] <- sol_mult[1]*(params@species_params$w_mat[1]/params@species_params$w_mat[i])^(params@lambda-params@n)
...
true_sol <- true_sol * params@cc_pp[params@w_full>=params@w[1]][ceiling(0.7*myno_w)]/colSums(true_sol)[ceiling(0.7*myno_w)]

#sim <- project(params, t_max=500, effort = 0)
##sim <- project(params, t_max=500, effort = 0,initial_n=true_sol)
##plot(sim)
myinteraction2 <- matrix(1,nrow=myno_sp,ncol=myno_sp)

params2 <- set_trait_model(interaction=myinteraction2,r_pp = myrpp,no_sp = myno_sp, min_w_inf = 10, max_w_inf = 10^5,
                          w_pp_cutoff = 0.25*10,z0pre = 0,p=2/3,alpha=myalpha,no_w=myno_w,q=myq)
# should w_pp_cutoff and max(w_inf) be multiplied by 0.25
params2@mu_ext <- large_predation(params,WW=0.25*max(params@species_params$w_inf))

for (i in 1:no_s){
    params2@species_params$r_max[i] <- true_sol[i,params@species_params$w_min_idx[i]]*
        hbar[i]*params@species_params$w_min[i]^params@n
}


sim <- project(params2, t_max=50, effort = 0,initial_n=true_sol)
plot(sim)
...
ss <- getSteadyState(params)
plot(params@w,ss[1,],log="xy",type="l",ylim=c(10^(-30),10^(2)))
for (i in 2:dim(sim@n)[2]){
    lines(params@w,ss[i,],col="blue")
}
...
plot(params@w,params@w*colSums(sim@n[1,,]),log="xy", type="l")
lines(params@w,params@w*sim@n_pp[dim(sim@n_pp)[1],params@w_full>=params@w[1]],col="red")

abline(v=params@species_params$w_mat[1])

# plot analytic solution
plot(params@w,params@w*colSums(true_sol),log="xy", type="l")
lines(params@w,params@w*params@cc_pp[params@w_full>=params@w[1]],col="red")
plot(params@w,params@w*params@cc_pp[params@w_full>=params@w[1]],col="red", log="xy")
abline(v=params@species_params$w_mat[1])

# get solution
# rescale solution so community fits power law
# use rescaled solution to determine rmax
# compare with mizer when interaction=0, and plankton&pred are on full
# compare with mizer when interaction=1, and plankton&pred are cut back
