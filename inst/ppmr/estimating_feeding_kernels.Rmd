...
The sizes of the actual observed prey items are seen as samples from this probability distribution.
...
### Biomass densities
Instead of the number density $N_w(w|w')$, we can look at the biomass density
$B_w(w|w')$ so that $B_w(w|w')dw$ is the total biomass of prey items with sizes
between $w$ and $w+dw$. This makes some sense because as far as the predator is
concerned, it is the amount of biomass that counts, not the number of 
individuals that are needed to make up that biomass. The number density and the
...
of a predator of size $w'$ comes from a prey item of size $w$.
The relation to the earlier density is
$$
b_w(w|w') \propto w\ f_w(w|w').
$$

To see how this biomass density is related to the observations consider that
instead of recording one observation for each prey individual, we could in
principle make one observation for each unit of biomass in the stomach. This
would correspond to splitting each single observation for a prey time of size
$w$ into $w$ separate observations. This new larger set of observations could
then be seen as a sample from the biomass density $b_w(w|w')$.
...
predator/prey mass ratio $l = w'/w$.
```{r}
ppmr <- ppmr %>% 
...
binsize <- (max(ppmr$l) - min(ppmr$l)) / (no_bins - 1)
breaks <- seq(min(ppmr$l) - binsize/2,
...
histograms are estimates of the normalised number density $f_l(l)$ and the
normalised biomass density $b_l(l)$.
...
$$
Due to the way the parameters $l_min$ and $l_max$ enter in the normalisation of
the probability density, the maximum likelihood will be attained at the 
largest possible value for $l_min$ and the smallest possible value for $l_max$,
i.e., they are determined by the smallest and largest value appearing in the
dataset. It remins only to estimate the rate parameter $\alpha$.
...
plot(l, dtexp(l, 0.1, 5, 1, 25, 1), type = "l")
```
...
shifted_normaldens <- plyr::ddply(ppmr, "Species", function(df) {
  data.frame( 
    l = grid,
