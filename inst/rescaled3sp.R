...
s_params <- set_scaling_model(no_sp = 3, no_w = 200)
...
#nsave <- RF$n
#save(nsave, file = "inst/spAlt.RData")
load( file = "inst/spAlt.RData")


#n_ppsave <- RF$n_pp
#save(n_ppsave, file = "inst/spAltpp.RData")
load( file = "inst/spAltpp.RData")


plot(project(used_params, t_max=15, effort = 0, initial_n = nsave, t_save = 1, initial_n_pp = n_ppsave))
...
#74 found an saved a non-scale invarant steady state of the three species case.
