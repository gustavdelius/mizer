...
#rmult <- 10^(2*runif(11)-1)
rmult <- c(0.3133014, 1.7311255, 0.5567736, 8.7692104, 2.3060917, 0.7723255, 3.8792129, 5.7460053,
          0.6637151, 6.8325331, 5.3335027)
rmult <- 1000^(2*runif(11)-1)
...
# changed to see fluxuations about steady state beta <- 100
# beta =30 kills the system. beta =50 or 100(old defailt) leads to stability
# when beta = 500 our pertuburbed initial condition takes 50 years + to 
# get to something fluxuating near the steady state. Here we are pertubing the initial 
# conditions by multiplifying N_i(w) of the initial condition by 10^r where 
# r was selected uniformly from [-1,1] at random

# beta = 10^12 -> extinction
# beta = 10^11 largest 2 species survive close to pow law, others go extinct
# beta = 10^10 fully stable system, still with upturn on rhs, and with stability because 
# they all feed on plakton
# beta = 10^9, similar
# actually whats going on here is that the randomly pertubed initial 
# condition is itself stable, as is any initial condition that yields positive 
...
for (i in (1:no_sp)){
  n_output_mod[i,] <- rmult[i]*n_output[i,]
}
t_max <- 500
sim <- project(params, t_max=t_max, dt=0.05, t_save=t_max/100 ,effort = 0, 
               initial_n = n_output_mod, initial_n_pp = initial_n_pp)
plot(sim)
no_trophic_lvls <- log(max(w_inf)/min(params@w_full))/log(beta)

###
#' Compare the final state (in blue and green) to the initial condition (in black and grey).
plot(w,w^2*sim@n[1,1,],log="xy",type="l",ylim=c(10^3,10^14), 
...
gb <- getBiomass(sim)
plot(gb[,1],log="y",type="l",ylim=c(min(gb),max(gb)))
for (i in (2:no_sp)){
  lines(gb[,i])
}

#finals1 <- sim@n[dim(sim@n)[1],,]

finals2 <- sim@n[dim(sim@n)[1],,]

plot(w,finals2[1,]-finals1[1,],log="xy")

plot(w,log(finals2[1,])-log(finals1[1,]))
