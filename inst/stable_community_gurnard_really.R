...
knife_edge_size <- rep(10^10,no_sp)
...
#h[rep_idx] <- lam*params_data_NS$h[gurn_sp]+(1-lam)*h[rep_idx]
# need to think more about how to get this h
h[rep_idx] <- lam*19.37727+(1-lam)*h[rep_idx]
#ks[rep_idx] <- lam*params_data_NS$ks[gurn_sp]+(1-lam)*ks[rep_idx]
# similary obtained by running mizer params on standard north sea setup
ks[rep_idx] <- lam*0.266+(1-lam)*ks[rep_idx]
...
# dont know what alpha of gurnard is
...
mu0_g <- ((1-f0) * sqrt(2*pi) * kappa * gamma * sigma *
            (beta^(n-1)) * exp(sigma^2 * (n-1)^2 / 2))[rep_idx]
...
# actually got the gurnard to coexist without chi, by changing max_egg_size from -2 to -1.9
