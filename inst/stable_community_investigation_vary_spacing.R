...
plot(w,sim@n[1,1,],log="xy",type="l",ylim=c(10^(-1),10^18))
...
params@chi <- 0.1
#@# params@ddd <- nn^(params@chi)
params@ddd <- mm^(params@chi)
sim <- project(params, t_max=300 ,effort = 0, initial_n = n_output)
plot(sim)
gb2 <- getBiomass(sim)
# here we have chi=0.1, no_sp=11, dist_sp=0.4. gb is th biomass attributed with normal sim
# gb2 is biomass associated with sim where the biomass of species 8 has doubled. We compare
# inititial and final
gb[dim(gb)[1]]/gb[1,]
