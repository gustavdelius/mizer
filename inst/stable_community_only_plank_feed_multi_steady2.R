...
#' add fishing on one species, and see how changing the effort alters things
#' increase beta, so everything is feeding from plankton, and show 
#' that there are multiple steady states
#' experiment wiith cases like those with two identical species, and see 
#' if there are multiple steady states
#' use newton raphson and eigenvalue methods
#' do random pertubations always get pulled to the same steady state
#' 
...
#' 
#' I found another example where changing the initial condition causes the system 
#' to organize into a different steady state. In this case (stable_community.R but with beta=200) 
#' the fish can eat each other, but we still seem to see doubling the abundance 
#' of the first species causes the system to organize into a different steady state. 
#' 