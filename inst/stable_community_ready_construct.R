...
idx_start <- sum(w<=min(w_mat))
idx_stop <- sum(w<=max(w_mat))
...
plot(w,colSums(n_output_new),log="xy")
lines(w,kappa*w^(-lambda),col="red")
abline(v=min(w_mat))
abline(v=max(w_mat))
...
plot(w,w^2*sim@n[1,1,],log="xy",type="l",ylim=c(10^7,10^13), 
     ylab="Biomass in logged bins")
lines(w,w^2*n_output_new[1,],col="blue")
for (i in (2:no_sp)){
  lines(w,w^2*sim@n[1,i,],col="black")
  lines(w,w^2*n_output_new[i,],col="blue")
}
fish_indices <- (length(params@w_full)-length(w)+1):length(params@w_full)
lines(w,w^2*sim@n_pp[1, fish_indices],col="grey")
lines(w,w^2*sim@n_pp[dim(sim@n)[1], fish_indices],col="green")
