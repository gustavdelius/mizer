...
The first central assumption is that an individual can be characterized by its weight $w$ and its species number $i$ only. The aim of the model is to calculate the size- and trait-spectrum $N_i(w)$ which is the density of individuals such that $N_i(w)dw$ is the number of individuals in the interval $[w:w+dw]$. The dimensions of the size spectrum are numbers per weight per volume.  From the individual-level processes of growth and mortality, an evolution equation for the size spectrum of each trait group can be derived \cite{datta_jump-growth_2010},

\begin{equation}
  \label{eq:MvF}
  \frac{\partial}{\partial t} N_i(w,t) + \frac{\partial }{\partial w} \left(g_i(w) N_i(w)\right)
   = -\mu_i(w) N_i(w)+\lambda_i(w),
\end{equation}

where individual growth $g_i(w)$ is determined by the availability of food from the other species and from background resources, and mortality $\mu_i(w)$ is determined by predation by the other species. 
The birth rate $\lambda_i(w)$ is non-zero only in a small size interval corresponding to the size of eggs produced by the mature fish. This equation can be seen to be  generalisation of the McKendrick-von Foerster equation. The equation is supplemented by a boundary condition which sets 
$N_i(w)=0$ for $w<w_{\text{min}.i}$ where $w_{\text{min}.i}$ is the weight of the smallest individual.
...
The available food (mass per volume) for a predator of weight $w$ is determined by integrating over all species and the background resources weighted by the size selection function (Equation~\ref{eq:4}): 

\begin{equation}
  \label{eq:1}
  E_{a.i}(w) = \int \left( \sum_{j} \theta_{ij} N_j(w_p) +\sum_a \theta^R_{ia}N^R_a(w_p) \right) \phi(w_p/w) w_p \, dw_p.
\end{equation}

where $\theta_{ij}$ is the preference of species $i$ for species $j$ and $\theta^R_{ia}$ the
preference of species $i$ for resource $a$. The food actually encountered $E_{e.i}$ (mass per time) depends on the search rate (volume per time) which is assumed to scale with individual weight as $\gamma_i w^q$:
...
\subsection{Growth and Reproduction}
\label{sec:growth}
...
The total production rate of eggs $R_{p.i}$ (number per time) is found by integrating the energy allocated to reproduction over all individuals.

\begin{equation}
  \label{eq:Rp}
  R_{p.i} = \frac{\epsilon}{w_{\text{min}.i}} \int N_i(w)  (\alpha f_i(w) h_i w^n - k_{s.i} w^p) \psi_i(w) \, dw,
\end{equation}

where $w_{\text{min}.i}$ is the egg weight and $\epsilon$ is the efficiency with which energy
allocated to reproduction is converted to egg biomass. 
...
The effective egg production rate $R_i$ leads to a positive birth rate $\lambda_i(w)$ (see eq. \eqref{eq:MvF}) in the immediate neighbourhood of $w=w_0$ so that $\int\lambda_i(w)=R_i$ (Putting all eggs in at exactly $w=w_0$ would lead to a mathematical singularity).
...
The background resource spectra $N^R_a(w)$ represents food items for the smallest individuals (smaller than $\beta w_0$). The temporal evolution of each size group in the resource spectra is described using semi-chemostatic growth:
\begin{equation}
  \label{eq:nb}
  \frac{\partial N^R_a(w,t)}{\partial t} = r_0 w^{p-1} \Big[ \kappa_R w^{-\lambda} - N^R_a(w,t) \Big] - \mu_{p.a}(w) N^R_a(w,t),
\end{equation}
where $r_0 w^{p-1}$ is the population regeneration rate \citep{fenchel_intrinsic_1974,savage_effects_2004} and $\kappa w^{-\lambda} = \kappa w^{-2-q+n}$ the carrying capacity. The predation death rates
$\mu_{p.a}$ are given by the same expression as in \eqref{eq:mup} but with $\theta_{ja}^R$ instead
of $\theta_{ji}$.
In the current version of mizer the regeneration rate and the carrying capacity are identical for all resource spectra.
...
While size-spectrum models are formulated in continuous time and contiuous size, for implementation
in the computer, both time and weight need to be discretised. 

We introduce a discrete set of weights $w_\omega$ and a discrete sets of times $t_\tau$, with the indices $\omega$ and $\tau$ being natural numbers. The continuous
functions $N_i(t,w)$ are then replaced by an array of values $n[\tau,i,\omega]=N_i(t_\tau,w_\omega)$. Similarly the resource abundances $N_a^R(t,w)$ are represented by an array $n_R[\tau,a,\omega]=N_a^R(t_\tau,w_\omega)$.
...
The single resource spectrum only extends to the start of the community spectrum.
...
\subsection{The interaction matrices}
\label{sec:interaction_matrix}

The interaction matrices $\theta_{ij}$ and $\theta_{ia}^R$ were introduced in equation \eqref{eq:1} 
and describe the strenght of interaction between predators and prey or resources respectively.
This can be viewed as a proxy for spatial interaction
e.g. to model predator-prey interaction that is not size based.

The predator-prey interaction matrix $\theta_{ij}$ also enters the expression \eqref{eq:mup} for the 
mortality due to predation.

The matrix is square with every element being the interaction between a pair of species. The dimensions of the matrix therefore equal the number of species.
...
The predator-resource interaction matrix $\theta^R_{ia}$ only enters the expression \eqref{eq:1}
for the amount of available food. It is a rectangular matrix. It has as many rows as there are
species and as many columns as there are resources. 

The interaction matrices must be of type \class{array} or \class{matrix}. 
...
It should be noted that the order of species in the interaction matrices has to be the same as the order in the species parameters data.frame.
Although you can specify the dimnames of the predator-prey interaction matrix, these names are overwritten by the species names from the species parameters data.frame
inside the \class{MizerParams} constructor.

If an interaction matrix is not specified to the \code{MizerParams()} constructor,
a default is used in which all entries are set to $1$. Furthermore in this case it is assumed that
...
<<<<<<< HEAD
We also compare the simulated values of the LFI to a community target based on achieving a high proportion of the unexploited value of the LFI if $0.8 LFI_{F=0}$ \citep{rochet_does_2011}.
=======
