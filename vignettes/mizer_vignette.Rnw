...
\pkg{mizer} is able implement all three model versions using the same set of tools.
...
The results of the simulations can then be explored using a range of plots and summary methods, including plots of size spectra and the calculation of community indicators such mean weight in the community and the the slope of the size spectrum.
...
\end{equation}
where $\beta_i$ is the preferred predator-prey mass ratio and $\sigma_i$ the width of the weight selection function for predators of species $i$.
...
\subsection{Growth}
\label{sec:growth}
The consumed food $f_i(w) h_i w^n$ is assimilated by an efficiency $\alpha$ and used to fuel the needs for standard metabolism and activity $k_{s.i} w^p$. The remaining available energy, $\alpha f_i(w) h_i w^n - k_{s.i} w^p$, is divided between growth and reproduction by a function of weight that grows rapidly from near zero to near one at around the weight of maturation:
...
\begin{figure}
<<label=print_plot_comm_m2, echo=FALSE, fig.height=4>>=
...
Which can be plotted using (Figure~\ref{fig:plot_relative_comm_abund_2}):

<<label=plot_relative_comm_abund_2, eval=FALSE>>=
...
\begin{figure}
<<label=show_plot_relative_comm_abund_2, eval=FALSE, fig.height=4>>=
<<plot_relative_comm_abund>>
@
\caption{Relative abundances from the unfished (dashed line) and fished (solid line) trait based model.}
\label{fig:plot_relative_comm_abund_2}
\end{figure}
...
\begin{figure}
<<label=show_plot_relative_comm_abund_industrial, eval=FALSE>>=
...
The essential columns of the species parameters data.frame that have no default values are: \args{species}, the names of the species in the community; \args{w\_inf}, the asymptotic mass $W$ of the species; \args{w\_mat}, the mass $w_m$ at maturation (see eq.\eqref{eq:psi}); \args{beta} and \args{sigma}, the predator-prey mass ratio parameters $\beta$ and $\sigma$ (see eq.\eqref{eq:4}); stock-recruitment parameters (by default, the stock-recruitment function is a Beverton-Holt type and so, unless a different SRR function is used, a \args{r\_max} column must be provided - see Section~\ref{sec:srr} for more details).

Essential columns that have default values are: \args{k}, the activity coefficient (default = 0); \args{alpha}, the assimilation efficiency $\alpha$ (default = 0.6); \args{erepro}, the reproductive efficiency $\epsilon£ (default value = 1); \args{w\_min}, the size $w_0$ of recruits (default value is the smallest size of the community size spectrum); \args{sel\_func}, the name of the fishing selectivity function (default value = "knife\_edge"); \args{gear}, the name of the fishing gear that catches the species (default value is the name of the species in the \args{species} column); \args{catchability}, the catchability of the fishing gear on that species (default value = 1). Additionally, columns that contain the selectivity function parameters are needed. As mentioned, the default selectivity function is a ``knife\_edge'' function. This has an argument \args{knife\_edge\_size} that determines the knife-edge position and has a default value of \args{w\_min}.
...
Standard metabolism is used in calculation of the growth of individuals as $k_{s,i} w^p$ (see Section~\ref{sec:growth}).
...
\begin{table}[ht]
	...
	%\begin{tabularx}{r|X|X}
	    ...
	    \hline
	    \args{species} & Name of the species & Compulsory (no default)\\
	    \args{w\_inf} & The asymptotic mass of the species $W$ & Compulsory (no default)\\
	    \args{w\_mat} & Maturation mass $w_m$. Used to calculate values for $\psi$. & Compulsory (no default)\\
	    \args{beta} & Preferred predator prey mass ratio $\beta$ & Compulsory (no default)\\
	    \args{sigma} & Width of prey size preference $\sigma$ & Compulsory (no default)\\
        \args{h} & Maximum food intake rate $h$. & Optional. If not provided, it is calculated using the \args{k\_vb} column. \\
        \args{k\_vb} & The von Bertalanffy $K$ parameter. Only used to calculate \args{h} if that column is not provided & Optional (no default) but either \args{h} or \args{k\_vb} must be provided.\\
	    \args{gamma} & Volumetric search rate $\gamma$.  & Optional. If not provided, it is calculated using the \args{h} column and other parameters.\\
        \args{ks} & Standard metabolism coefficient $k_s$ & \args{h} * 0.2\\
        \args{z0} & Background mortality $\mu_b$ (constant for all sizes).   & Optional. If not provided then \args{z0} is calculated as \code{z0pre * w\_inf \^{} z0exp}. \args{z0pre} and \args{z0exp} have default values of 0.6 and -1/3 respectively.\\
	    \args{k} & Activity coefficient & 0.0\\
	    \args{alpha} & Assimilation efficiency $\alpha$ & 0.6 \\
	    \args{erepro} & Reproductive efficiency $\epsilon$ & 1 \\
	    \args{w\_min} & The size class $w_0$ that recruits are placed in. & smallest size class of the species size spectrum \\
	    \hline
	    \multicolumn{3}{c}{Fishing gear parameters (see Section~\ref{sec:fishing_gear} for more details).} \\
	    \hline
	    \args{sel\_func} & The name of the selectivity function to be used.  & "knife\_edge".\\
	    \args{gear} & The name of the fishing gear that selects the species. At the moment a species can only be selected by one gear. & Name of the species \\
	    \args{catchability} & The catchability of the fishing gear. & 1\\
	    ...
	    other columns & Other parameters used by the selectivity function.  &  The default "knife\_edge" selectivity function uses a column "knife\_edge\_size" for which the default is \args{w\_mat} (see Section~\ref{sec:fishing_gear}).\\
	    \hline
...
The name of the selectivity function is given by the \args{sel\_func} column in the species parameters data.frame.
...
\begin{table}[ht]
	...
	\hline
	\args{min\_w} & The smallest size of the species community size spectrum. Note that this can be different from \args{w\_min} in the species parameter data.frame. & 0.001 \\
	\args{max\_w} & The largest size of the species size spectrum. & The largest \args{w\_inf} in the species parameters data.frame * 1.1 \\
	\args{no\_w} & The number of size bins in the species size spectrum. & 100 \\
	\args{min\_w\_pp} & The smallest size of the background size spectrum. & 1e-10 \\
	\args{no\_w\_pp} & The number of size bins in the background size spectrum. & \args{round(no\_w)} * 0.3 \\
	\args{n} & The scaling of intake. & 2/3 \\
	\args{p} & The scaling of standard metabolism. & 0.7 \\
	\args{q} & The search volume exponent. & 0.8 \\
	\args{r\_pp} & The growth rate $r_0$ of the resource (the background spectrum). & 10 \\
	\args{kappa} & The carrying capacity $\kappa_R$ of the background spectrum. & 1e11 \\
	\args{lambda} & The exponent $\lambda$ of the background spectrum. & 2+\args{q}-\args{n} \\
	\args{w\_pp\_cutoff} & The cut off size of the background spectrum. & 10 \\
    \args{f0} & The feeding level $f_0$ of small individuals feeding mainly on the background resource. Used to calculate \args{h} and \args{gamma} if they are not provided in the species parameter data.frame. & 0.6\\
    \args{z0pre} & If \args{z0}, the mortality from other sources, is not a column in the species data.frame, it is calculated as \code{z0pre*w\_inf\^{}z0exp}. & 0.6 \\
    \args{z0exp} & See \args{z0pre} & \args{n} - 1\\
	\end{tabularx}
...
\begin{description}
    ...
    \item[interaction] The interaction matrix (see Section~\ref{sec:interaction_matrix}). The default is a matrix of 1s.
...
Note that The \args{n} and \args{n\_pp} slots have one more row than the \args{effort} slot. This is to the store the initial populations.
...
The examples will focus on fishing effort can be specified in different ways.
...
We will use the \args{params} object that was created above in the \class{MizerParams} example above in which each species is caught by a separate gear.
...
You can also see in Figure~\ref{fig:plot_food_demo} that each species is has different fishing selectivity (see the Total fishing mortality panel). Remember that the default setting for the fishing gears is a knife-edge gear where the knife-edge is positioned at the species \args{w\_mat} parameter.
...
\begin{figure}
<<label=print_plotFMort, echo=FALSE, fig.height=4>>=
<<plotFMort>>
@
...
Here, we will use the \args{params\_gears} object we created earlier which has four gears.
The names of the gears are \Sexpr{paste(unique(params_gears@species_params$gear), collapse = ", ")}.
...
\begin{figure}
<<label=print_plotBiomass, echo=FALSE, fig.height=4>>=
<<plotBiomass>>
@
...
\begin{figure}
<<label=print_plotSpectra_example, echo=FALSE, fig.height=4>>=
<<plotSpectra_example>>
@
...
where $l$ is the length of an individual, $S_l$ is the selectivity at length, $S2 = log(3) / (l50 - l25)$ and $S1$ = $l50 * S2$.
...
Note it would probably easier to add this data directly to the *.csv file and then read it in rather than type it in by hand like we do here:
...
Note that we don't set up a \args{gear} column to give specific gear names. This means that each species will be caught by a separate gear named after the species.
...
Here we use the year 1990 as the reference year. If we set the catchability of each species to be the same as the fishing mortality in 1990 then an effort of 1 in 1990 will result in the fishing mortality being what it was in 1990. The effort in the other years will be relative to the effort 1990.
...
One way around is this to project forward at a constant fishing mortality equal to the mortality in the first historical year until equilibrium is reached.
...
\begin{figure}
<<label=print_plot_ns_biomass, echo=FALSE, fig.height=4>>=
<<plot_ns_biomass>>
@
...
Here we look at some of the way the results of the simulation can be explored.
...
We also compare the simulated values of the LFI to a community target based on achieving a high proportion of the unexploited value of the LFI $0.8 LFI_{F=0}$ \citep{rochet_does_2011}.
...
Before we can running the simulations, we need to set up arrays of future effort.
We will continue to use effort relative to the level in 1990.
Here we make build on our existing array of relative to make an array for the first scenario. Note the use of the \code{t()} command to transpose the array. This because \R recycles by rows, so we need to build the array with the dimensions rotated to start with.
