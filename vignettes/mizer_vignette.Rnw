...
\title{Multispecies, trait-based and community size spectrum ecological modelling in \R (\pkg{mizer})\thanks{mizer version 0.2 authored by Finlay Scott (Cefas, Lowestoft, UK, Maritime Affairs Unit, IPSC, European Commission Joint Research Centre, IT), Julia L. Blanchard (University of Sheffield, UK) and  Ken. H. Andersen (DTU Aqua, Copenhagen, DK). Further improvements were made by Gustav Delius, Richard Southwell and Richard Law (University of York), which resulted in the current version of mizer.}}
...
This vignette starts by summarising the principles and assumptions of size spectrum models. The \pkg{mizer} package is then introduced by showing how to set up the simplest types of size spectrum models: the community and trait-based model. These sections give a basic overview of the classes and methods used by \pkg{mizer}. Simple examples are given that demonstrate how trophic-cascades can be simulated. There then follows a more detailed description of the \pkg{mizer} classes and methods using a multispecies model of the North Sea as an example. Next, there is a detailed example of a multispecies size spectrum model of the North Sea, including running projections using historical fishing patterns. After this, we introduce the scale-invariant trait-based model, which can be thought of as a modified version of the trait-based model that can be organized into a well understood steady state without the need to impose external stock-recruitment relationships. Next we describe how species from the scale-invariant can act as a type of background into which  explicit species of particular interest can be added. Finally we describe improvements that have been made to the previous version of mizer, and discuss some technical details in the appendix.
...
Background mortality $\mu_{b .i}(w)$ can be set freely in mizer. By default
it is assumed to be constant within a species and inversely proportional to 
generation time \citep{peters_ecological_1986}:
\begin{equation}
  \label{eq:mu_b}
...
\begin{equation}
  \label{eq:nb}
  \frac{\partial N_R(w,t)}{\partial t} = r_0 w^{p-1} \Big[ c_p (w) - N_R(w,t) \Big] - \mu_p(w) N_R(w,t),
\end{equation}
where $r_0 w^{p-1}$ is the population regeneration rate \citep{fenchel_intrinsic_1974,savage_effects_2004} and $c_p(w)$ is the carrying capacity, which takes the value $\kappa w^{-\lambda} = \kappa w^{-2-q+n}$ by default. 
...
\section{Scale-invariant, trait-based model}
\label{sec:scale_invariant_trait_based_model}
...
by making the controlling parameter $R_{max.i}$ in Equation~\ref{eq:R} sufficiently 
small. Although coexistence can be achieved in this way, it can necessitate 
...
<<plot_scale_sense2>>
@
\caption{Biomass plots following introduction of fishing (1) in the trait-based 
model from Section~\ref{sec:trait_based_model}, and (2) in the scale-invariant 
model developed here.  There are ten species; $W$ is taken over a range 10 g
%to 100 kg, 
to 1000 g,
...
%$F = 0.75$ yr$^{-1}$.}
$F = 1$ yr$^{-1}$.}
\label{fig:plot_biomass_tb_vs_si}
\end{figure}

This and the next Section~\ref{sec:explicit} provide an alternative way of 
...
Figure~\ref{fig:plot_biomass_tb_vs_si} compares it with the trait-based model 
in Section~\ref{sec:trait_based_model} in the presence of a substantial
amount of fishing.  In the case of the trait-based model, all species
persist under fishing (Figure~\ref{fig:plot_biomass_tb_vs_si}.1).
In this instance, the species with largest maximum weight enters the 
fishery at about 
%1/25
half
% knife_edge = 100, max(w_mat) = 239
of its weight at maturation, and persists because
its stock-recruitment relationship is sufficiently strong to boost its 
rate of increase when rare. You can construct 
Figure~\ref{fig:plot_biomass_tb_vs_si}.1, using the following code:
...
more sensitive to fishing (Figure~\ref{fig:plot_biomass_tb_vs_si}.2).  
...
internal dynamics \cite{rossberg_2013}.
You can construct Figure~\ref{fig:plot_biomass_tb_vs_si}.2, using the 
following code:
...
to be near, after aggregating over species \cite{sheldon_1972}.  The significance 
...
model \cite{cuesta_2018}.  Related numerical simulations using  finite
number of species support the proposition that coexistence of multiple 
species is easier to achieve near the power law \cite{law_2016}.  
These properties readily extend to multicellular fish assemblages 
(Southwell, Delius unpublished results).
...
(see Section~\ref{sec:trait_based_model}) close to a scale-invariant form 
(see Section~\ref{sec:appendix}), for which the steady-state is known
analytically (Equation~\ref{eq:steady}).  For want of a better 
term, we call this model the \emph{scale-invariant}, trait-based model, 
with the caveat that the model is close to, but not exactly, scale invariant.  

Second, in the following 
Section~\ref{sec:explicit}, these background species are replaced 
...
{\bf What is scale invariance?}  
Consider a species with some characteristic weight (maximum weight $W$
will do for the purpose), and with a density 
function $N(w,t)$ that has dynamics given by the size-spectrum model 
(Equation~\ref{eq:MvF}).  Consider a second species with maximum 
weight $cW,$ where $c$ is a positive rescaling factor. Then Equation~\ref{eq:MvF} 
...
Here $\lambda$ is the exponent of the resource spectrum, and $n$ is the 
...
every species.  Strictly speaking, it could only apply to Equation~\ref{eq:MvF} 
if there was a continuum of maximum weights, with a species at
every $W > 0$.  Clearly, this cannot be the case in a multispecies spectrum with 
a finite set of species, because the $W$s are typically separated by finite 
...
%These boundaries are dealt with by special assumptions described below, 
%to retain the property of 'close-to-scale-invariance'.

%We avoid the problem of having to model arbitrarily small species by replacing them with the background resource, so the 
%smallest species we explicitly model still have something to eat. We avoid the 
%problem of having to model arbitrarily large species by choosing the background 
%mortality term $\mu_b (w)$ to emulate the effect of predators larger than the 
%explicitly modelled species.
  
{\bf Why bother with scale invariance?} 
...
\cite{sheldon_1972}.  The existence of this remarkable regularity, plus the 
...
construct \emph{background} assemblages that facilitate coexistence of 
observed fish species (\emph{foreground} species), without calling for 
strong stock-recruitment relationships. 


\subsection{Implementing the scale-invariant model}

Here we describe how to run the scale-invariant model, and make some 
comparisons with the trait-based model in Section~\ref{sec:trait_based_model}. 
%Details about our scale-invariant model and its motivations are given in
%Section~\ref{sec:appendix}.

To help set up a scale invariant trait-based model, there is a wrapper function, \code{set\_scaling\_model()}. 
Like the \code{set\_community\_model()} 
and 
\code{set\_tait\_model()} 
...
(\args{no\_sp}), 
the minimum egg size %(\args{min\_egg}), 
the minimum maturity size of a species %(\args{min\_w\_mat}) 
and the minimum and maximum asymptotic sizes %(\args{min\_w\_inf}) 
and 
(\args{max\_w\_inf}) 
...
We make a plot of the biomass over time (Figure~\ref{fig:plot_scale_demo}) as follows:
...
<<plot_scale_sim>>
@
\caption{A plot of time vs biomass for the scale invariant trait-based model with no fishing.}
\label{fig:plot_scale_demo}
\end{figure}

One can see from Figure~\ref{fig:plot_scale_demo} that the system is 
...
to the power law (Figure~\ref{fig:plot_scale_demoBS}).
...
<<plot_scale_simBS>>
@
\caption{A plot of weight vs biomass density for the steady state of the scale invariant trait-based model with no fishing.}
\label{fig:plot_scale_demoBS}
\end{figure}



%By comparing Figure~\ref{fig:plot_scale_demo} with 
%Figure~\ref{fig:plot_trait_demo}, one can see that the output of the 
%scale-invariant, trait-based model is somewhat similar to that of the 
%previous trait-based model. The key difference
%is that a strong stock-recruitment relationship is present in the original 
%trait-based model. This allows the number of fish to increase rapidly, 
%initially, until the population growth is eventually halted 
%by the stock-recruitment relationship. If one removed the stock recruitment 
%relationship, the populations in the original trait based model would 
%continue to grow rapidly, but they are being \emph{held back} by the stock 
%recruitment relationship. The result is that the stock in the original 
%trait based model is somewhat insensitive to changes in the environment, 
%because its steady state is the result of a strong force for population 
%growth being halted by a strong breaking force (the stock-recruitment 
%relationship) . 



\subsection{Details of changes to the trait-based model}
\label{subsec:bullets}
...
scale invariance.  The new model is similar to the original trait-based model of 
Section~\ref{sec:trait_based_model}, but includes some modifications and restrictions to 
get assemblages that are close to scale invariance.  The changes are as follows.

%The scale invariant trait-based model corresponds to a special case 
%of the general size spectrum model described in Section~\ref{sec:principles}. 
%Essentially the scale invariant trait-based model is very similar to the 
%original trait-based model of Section~\ref{sec:explicit}, however there are 
%the following differences:

\begin{itemize}

 \item For the model to be scale invariant, the exponent of 
 metabolic intake $n$ needs to be equal to the exponent $p$ of metabolic loss.
 
 \item Scale invariance also requires that the exponent of the resource 
 ...
 \item 
 (Equation~\ref{eq:1}) Feeding depends on prey body size, but not on 
 ...
 \item (Equation~\ref{eq:psi}) We assume that the proportion of energy allocated to 
 reproduction $\psi_i(w)$ is given by the step-like function
 \begin{equation}
 \psi_i(w)=\begin{cases}
 0 & \text{if \ensuremath{w<w_{*.i}}}\\
 \left(\frac{w}{W_i}\right)^{1-n} & \text{otherwise}
 \end{cases}
 \end{equation}
 This can be achieved by taking the form given by Equation~\ref{eq:psi}, 
 and letting $u \rightarrow \infty.$  This is not a requirement 
 for scale invariance;  the assumption was made for the mathematical 
 reason that it is then possible to get a closed-form expression for 
 the steady state.

 \item (Equation~\ref{eq:Rp})
 ...
 adult weight \cite{andersen_2016}.  Scale invariance of egg 
 ...
 \item (Equation~\ref{eq:R})  A stock-recruitment relationship is not 
 ...
 scale-invariant trait-based model by setting the value of \args{r\_fac}. 
 Here \args{r\_fac} equals the maximum recruitment $R_{max.i}$ divided by the actual 
 recruitment $R_i$ at our steady state. By default the stock-recruitment 
 relationship is turned off, by setting 
 %\args{r\_fac} 
 to go to infinity. This has the effect of also making 
 $R_{max.i}\rightarrow \infty$, and so Equation~\ref{eq:R} is effectively 
 replaced with the simple relationship $R_i = R_{p.i}.$ 

 \item (Equation~\ref{eq:mu_b}) Beyond the upper limit of body weight. 
 The total mortality
 ...
 mortality.  Thus, we replace the background mortality in 
 Equation~\ref{eq:mu_b} with a more intricate expression $\mu_b(w)$ given by 
 Equation~\ref{eq:newmub}. 
 ...
 invariant pattern. 
 %We arrived at our expression for $\mu_b(w)$ by considering that in the inf use the expression $ \mu_{b.i}(w) = \max \{ \mu_0 w^{n-1} - \mu_{f.i}(w) - \mu_{p.i}(w),0 \}$ where 
 %\begin{equation}
 %\mu_0 = (1 - f_0)  \sqrt{2 \pi} \kappa  \gamma  \sigma \beta ^{n - 1} \exp\left(\frac{\sigma ^ 2  (n - 1) ^ 2} {2}\right),
 %\end{equation}
 %and $\mu_{f.i}(w)$ is the fishing mortality (as determined by the fishing gears), and %$\mu_{p.i} (w)$ is the predation mortality, given by (Equation~\ref{eq:mup}), where the abundance $N$ is the abundance at our steady state {\bf where shall we give N's formula ?}.  This is done to ensure that the total mortality rate on explicitly modeled species is $ \mu_0 w^{n-1},$ which scales with weight in the way required for scale invariance.
%We have to choose parameters so that $\mu _0$ is sufficiently large that $ \mu_{b.i}(w) = \mu_0 w^{n-1} - \mu_{f.i}(w) - \mu_{p.i}(w)\geq 0. $ 

\item (Equation~\ref{eq:nb}) The resource spectrum extends from the smallest weight at which an explicitly 
modeled fish may eat. We define the carrying capacity $c_p(w)$ of the 
background resource so when our system is at steady state, the 
combination of the background resource spectrum and the aggregate abundance of 
...
model is given by Equation~\ref{eq:cp}. 
%This is achieved choosing the carrying capacity of the background resource to be 
%\begin{equation} c_{p} (w)=
%   \begin{cases}
%      \min \left\{ \left(1+\frac{\mu_p(w)}{r_0 w^{p-1}} \right)\left( \kappa w^{-\lambda} - \sum_i N_i(w) \right),0\right\}, & \text{if}\ w<\min\{w_{*i}\} \\
%     0, & \text{otherwise}
%    \end{cases}
%\end{equation}
%Note that, much like in the background mortality term, we require that the aggregate abundance $\sum_i N_i(w)$ is less than or equal to the power law $\kappa w^{-\lambda} $ within the appropriate size range (i.e., for $w\leq\min\{w_{*i}\}$), 
%so that we do not have to use our $\min\{ *, 0\}$ operation to rewrite negative entries. We also arrange things so that as $w$ approaches $\min\{w_{*i}\}$ from below, the carrying capacity of the background resource declines to zero.  

\item By default the scale-invariant trait-based model is initialized at a 
 steady state (Equation~\ref{eq:steady}) (provided no fishing is being 
 performed), whereas the original trait based model is not initialized 
...
\section{Implementing explicit species in a scale-invariant background}
\label{sec:explicit}


\subsection{Introduction}

In Section~\ref{sec:scale_invariant_trait_based_model} we saw that using strong stock recruitment relationships, as implemented within the models discussed in sections \ref{sec:trait_based_model} and \ref{sec:north_sea_example} can lead to scenarios where the stock levels are unrealistically insensitive to changes in the environment. In order to fix this issue we want to create a model where species of interest can co-exist and interact without imposing a strong stock recruitment relationship. The key to our approach is to use species from the scale-invariant trait based model as a type of background or substrate, into which the heterogeneous species of specific interest (the \emph{foreground species}) can added. 

We begin with only the scale-invariant background, which is setup close to a steady state by the \code{set\_scaling\_model()} command. We add new species using the \code{addSpecies()} command, in such a way that the resulting system remains close to a steady state. To achieve this we lower the abundances of the background species in our candidate steady state to compensate for the addition of the new foreground species, so that the background species still have the same food levels, and growth rates, as they did in the original system. We also choose the abundance curve for the newly added species so that it will be time-invariant under Equation~\ref{eq:MvF}. Additional steps are taken to fine tune the resulting state to make sure that the background species are close to time-invariant under Equation~\ref{eq:MvF}, and that the quantity of fish growing up from the egg size is balanced by the rate of egg production.

%We get relatively close to a steady state by altering the abundance of the background species when a foreground species This is basically achieved by ensure that the background species experience the same growth rates and death rates as they did in the previous scale-invariant system. One can make an analogy here with other transplants of biological organs/organisms, where post transplant survival is based on the transplanted entity and its surrounds receiving similar stimuli before and after the transplant. We ensure that the background species encounter the same growth rates after the transplant as they did before by depressing the abundance of the background species to \emph{make room} for the newly added species, so that the aggregate abundance of all the species remains at the power law. We do not however ensure that the death rates of background individuals will be the same as those in the original system. However, if the newly added species is put in at low abundance, it will cause little change to the death rates of the pre-existing background species, and hence the abundances of the background species will remain stationary as they were before. The abundance curves of the newly added species are calculated under the assumption that their growth and death rates are the ones they would experience in the scale invariant system (as if they were at such a low abundance that self-interaction can be neglected). The more abundant the foreground species is, relative to the background, the worse our approximation to a steady state will be. However in practice, our initial condition often tends to be close enough to a true steady state that it will be attracted to it under the system dynamics.

\subsection{Adding a foreground species to a scale-invariant background}


Initially, when we start with the scale-invariant trait-based model, these species are organized, so that their aggregate abundance follows a power-law. This can be done using the \code{set\_scaling\_model()} command (discussed in Section \ref{sec:scale_invariant_trait_based_model}) as follows:
...
Let us introduce a species which we are interested in modelling explicitly into this background using our \code{addSpecies()} command. For the sake of example we shall introduce a red mullet, we start by specifying its parameters using a dataframe with a single row:
...
This code creates a new \class{MizerParams} object containing the original background species and the new foreground species, mullet. The \code{addSpecies()} command attempts to construct a new approximate steady state (\args{params\_out@initial\_n})
including the new species. 

%This is done by decreasing the abundance of the pre-existing background species, so that when the new foreground species is added (at an abundance such that the total biomass of individuals above weight \args{min\_w\_observed} is \args{biomass}), the aggregate abundance of all species in the system will still given by the power-law $\kappa w^{-\lambda}.$ 

%The abundance curve of the newly added species would correspond to a steady state in the case where its abundance (as controlled by the value of \args{biomass}) is negligible compared to the abundance of the background species. As one increases the value of \args{biomass} this approximation becomes less acceptable, and the initial condition given by \args{params\_out@initial\_n}
%will be further from a steady state. Another problem with setting the value of \args{biomass} too high is that if the abundance of the newly added species exceeds that of the target power-law, it will obviously be impossible that including background species as well will yield the power-law. The code will return an error message about negative abundance multipliers in this case. One way to fix such problems (about the background community being insufficiently abundant to support the value of \args{biomass} of the species one wishes to add) is to increase the value of \args{kappa} in the \code{set\_scaling\_model()} command. 

The stock recruitment relationship can be controlled by setting the value of \args{r\_fac}, as described in Subsection \ref{subsec:bullets}.
...
We can see from the resulting plot of time vs biomass that the system is initiated close to a steady state (Figure~\ref{fig:plot_mullet_demo}):
...
<<plot_mullet_sim>>
@
\caption{Example plot of time vs biomass for the scale invariant trait-based model plus mullet.}
\label{fig:plot_mullet_demo}
\end{figure}

We can also visualize the relative abundances of the different species (Figure~\ref{fig:plot_mullet_demo2}) with the following command:
...
<<plot_mullet_sim2>>
@
\caption{Example plot of weight vs abundance for the scale invariant trait-based model plus mullet, in the final state of our simulation.}
\label{fig:plot_mullet_demo2}
\end{figure}




%We can compare the growth curves of the mullet with the predictions from the von Bertalanffy model (Figure~\ref{fig:plot_mullet_growth}) using the following command:

%
%<<plot_mullet_growth, eval=FALSE>>=
%plotGrowthCurves(sim, species = "mullet", max_age = 15)
%@


%\begin{figure}
%<<label=print_plot_mullet_growth, echo=FALSE, fig.height=4, fig.keep='last'%>>=
%<<plot_mullet_growth>>
%@
%\caption{A plot showing the growth (time vs weight) of the mullet. The black curve shows the von Bertalanffy growth curve and the red curve shows the prediction of our model with mullet in a scale-invariant background}
%\label{fig:plot_mullet_growth}
%\end{figure}
 

\subsection{Adding another species}

In practical fisheries science one will have several species that one wishes to model explicitly. Further foreground species can be added into our system by repeated use of the \code{addSpecies()} command. Let us demonstrate this by adding hake into our system with background and mullet. We start by specifying the data for the hake:

%Again we see that the system is at a stable steady state without the need to impose stock recruitment relationships.
...
We visualize the initial condition, close to steady state (Figure~\ref{fig:plot_mullet_hake_spectra}) using the following command:
...
<<plot_mullet_hake_spectra>>
@
\caption{A plot of weight vs biomass density for the near steady state created by adding mullet and hake to a scale-invariant background.}
\label{fig:plot_mullet_hake_spectra}
\end{figure}

To see that this is close to a steady state we shall evolve the system and show how biomass changes over time (Figure~\ref{fig:plot_mullet_hake_biomass}):
...
<<plot_mullet_hake_biomass>>
@
\caption{A plot of time vs biomass showing our system with background species, mullet and hake,  evolving to a nearby steady state.}
\label{fig:plot_mullet_hake_biomass}
\end{figure}

The \code{steady()} command can be used to fine tune the initial condition to be very close to a steady state:
...
This results in a a configuration very close to a steady state, as we can see by evolving the system and plotting how biomass changes over time (Figure~\ref{fig:plot_mullet_hake_biomass_steady}):
...
<<plot_mullet_hake_biomass_steady>>
@
\caption{A plot of time vs biomass showing our system with background species, mullet and hake evolving from our fine tuned nearby steady state.}
\label{fig:plot_mullet_hake_biomass_steady}
\end{figure}


%\subsection{Adding fishing}

%In order to model the effect of fishing one could simply take the previous setup, and turn on fishing effort, but that would likely lead to the extinction of some species. The reason is that the system was setup to be at steady state for a system without any fishing, and without a strong stock recruitment relationship imposed the system reacts more sensitively to dramatic changes in the environment than the models in sections \ref{sec:trait_based_model} and \ref{sec:north_sea_example}. 

%In order to model the effects of fishing in our systems we organize include fishing effort within the \code{addSpecies()} command. This means that the fishing mortality is accounted for when the abundance curves of the newly added species are computed for the approximate steady state in the resulting \class{MizerParams} object. Including the effort during the system setup in this way emulates the way real marine systems reorganize themselves in such a way that they can sustain themselves under a moderate degree of fishing.

%\subsection{Comparing fishing gears}

%The scenario discussed just previously corresponds to a system in the Mediterranean sea where * fishing gear is used to catch mullet and hake. One focus of {\bf Assessment of the relative catch performance of hake, red mullet and striped
%red mullet in a modified trawl extension with T90 netting} was to determine the effect using a T90 extension to the fishing gear to alter its selectivity curve. We can see the effects of such a change as follows.  

%---------------------------------------------------

\section{Improvements to the previous version }
\label{sec:news}

Version 0.2 of mizer was authored by Finlay Scott, Julia Blanchard and Ken Andersen. In January 2017 Gustav Delius and Richard Southwell took over the job of improving mizer and made the following additions:

\begin{itemize}

\item {\bf Applied spectral methods to speed up integration:} Each time step mizer has to compute integrals to determine the growth rates and death rates. These integrals are convolution integrals. The mizer code was modified so these integrals are performed using spectral methods (using the fast Fourier transform). This led to a dramatic speedup of mizer.

\item {\bf Used C++ code for faster numerical solving of the PDE:} Used C++ for the inner loop in the project method for extra speed.

\item {\bf Improved plotting:} Added new plotting methods, fixed bugs, and added a progress bar while the simulation is running.

\item {\bf Added the scale-invariant model:} Added a function that computes parameters and a steady state for scale free trait-based models.

\item {\bf Allowed explicit species to be added to the a scale-invariant background:} Added a function that adds news species to a scale-invariant background, and computes an approximately steady state close to the power law.

\item {\bf Created web interface:} Made a shiny app to allow people to use mizer through a web browser without having to install the software.

\item {\bf Created tutorial videos:} Made videos explaining how to install and use mizer.
...
\subsection{A steady state solution}
...
\begin{equation} M_i (w)=
   \begin{cases}
      1, & \text{if}\ w<w_{*i} \\
      \frac{1}{\left(1-\left(\frac{w}{W_{i}}\right)^{1-n}\right)}\left(\frac{1-\left(\frac{w}{W_{i}}\right)^{1-n}}{1-\left(\frac{w_{*i}}{W_{i}}\right)^{1-n}}\right)^{\frac{1}{1-n}\left(\frac{\mu_{0}}{\hbar}\right)}, & \text{otherwise}
    \end{cases}
\end{equation}
and 
\begin{equation}
\label{eq:mu0}
 \mu_0 = (1 - f_0)  \sqrt{2 \pi} \kappa  \gamma  \sigma \beta ^{n - 1} \exp\left(\frac{\sigma ^ 2  (n - 1) ^ 2} {2}\right). 
 \end{equation}
As the number of species becomes large we have that $\sum_i N_i (w)$ is approximately proportional to $\kappa w^{-\lambda},$  $\forall w \in [\min \{w_{*i} \} , \max \{w_{*i} \}].$ The constant $H$ is chosen so that $\sum_i N_i (w)$ is approximately equal to $\kappa w^{-\lambda}$ $\forall w \in [\min \{w_{*i} \} , \max \{w_{*i} \}].$
This steady state is special because it is invariant under the type of rescaling described above. 
We also have to choose parameters in such a way that Equation~\ref{eq:Rp} holds. However, if it holds for one species, then it holds for all species due to scale-invariance.
Although the original trait based model had a similar level of simplicity, and a partial description of something like a steady state was possible, a careful reading of  Section 3 of \cite{hartvig_food_2010} reveals that it was in fact impossible to setup such a system so that the average number of offspring produced over an individuals lifespan is one, for each species, without imposing a stock recruitment relationship. This is part of the reason why stock recruitment relationships were imposed on previous models. However we believe that the stock recruitment relationships should emerge out of the life histories which size spectrum models claim to be able to model, rather than being imposed for outside the model.


\subsection{Background mortality in the scale invariant model}
...
\begin{equation}
 \label{eq:newmub}
  ...
  \end{equation}
  where $\mu_0$ is given by Equation~\ref{eq:mu0}, and $\mu_{p.i} (w)$ is the predation mortality, given by Equation~\ref{eq:mup}, where the abundance $N$ is the abundance at our steady state, given by Equation~\ref{eq:steady}.  This is done so that (provided we choose parameters so $ \mu_{b.i}(w) = \mu_0 w^{n-1}  - \mu_{p.i}(w)\geq 0. $) 
...
\subsection{Background resources in the scale invariant model}

We define the carrying capacity $c_p(w)$ of the background resource so when our system is at steady state, the 
combination of the background resource spectrum and the aggregate abundance of the explicitly modeled species is the power law $\kappa w^{-\lambda}.$ 
This is achieved choosing the carrying capacity of the background resource to be 
\begin{equation} 
\label{eq:cp}
...
%We also arrange things so that as $w$ approaches $\min\{w_{*i}\}$ from below, the carrying capacity of the background resource declines to zero.
...
for the project MINOUW (\url{http://minouw-project.eu/}).
