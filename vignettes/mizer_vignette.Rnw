...
\begin{equation}
  \label{eq:nb}
  \frac{\partial N^R_a(w,t)}{\partial t} = r_0 w^{p-1} \Big[ \kappa_R w^{-\lambda} - N^R_a(w,t) \Big] - \mu_p(w) N^R_a(w,t),
\end{equation}
where $r_0 w^{p-1}$ is the population regeneration rate \citep{fenchel_intrinsic_1974,savage_effects_2004} and $\kappa w^{-\lambda} = \kappa w^{-2-q+n}$ the carrying capacity. In the current version of
mizer the regeneration rate and the carrying capacity are identical for all resource spectra.
